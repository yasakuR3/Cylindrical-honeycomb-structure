...
def main():
    ...
    # === 3D プロット（点群 → 線グラフ：ワイヤーフレーム表示） ===
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection='3d')

    # 三角形インデックスから一意な"辺"集合を作る
    edges = set()
    for tri in triangle_indices:
        # tri は ((h,i,j), (h,i,j), (h,i,j)) のタプル
        k0, k1, k2 = tri
        for a, b in ((k0, k1), (k1, k2), (k2, k0)):
            # 順序に依らず一意にするため sort してタプル化
    ...
    for (key1, key2) in edges:
        ...
        x1, y1, z1 = hanikamu[h1, i1, j1, :]
        x2, y2, z2 = hanikamu[h2, i2, j2, :]

        ax.plot([x1, x2], [y1, y2], [z1, z2], linewidth=0.8)
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Cylindrical honeycomb (wireframe)')
